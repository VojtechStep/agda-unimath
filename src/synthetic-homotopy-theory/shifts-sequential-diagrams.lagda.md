...
We first use the above lemma, which says that the left cocone is homotopic to
`c[1][-1]`, and then we use the fact that unshifting is a retraction.
...
The proof is by induction on `k`; for the base case, observe that `A → A[0]` is
